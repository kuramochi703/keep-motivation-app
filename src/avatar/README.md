...
| --- | --- | --- | --- | --- | --- |
| しずみこみ | 4サイクル放置 | 4 / 58 | 閉じる | 伏せて動かない（`Sink`）・あぶら汗 | どんより |
| ぐったり | 3サイクル放置 | 8 / 58 | 閉じる | うずくまる（`Slump`）・あぶら汗 | どんより |
...
- エフェクトは3種類で、中身は `Effects.tsx`。どの気分で出すかは `MOODS` の `glow` / `motes` / `gloom`
  - **輝き**（`Glow`）: 背中の後光（ゆっくり回る光の筋）＋ぱっと光る星 ✦ ＋からだの金色の発光
  - **光の粒**（`Motes`）: 足元から同じ向きに回りながら立ちのぼり、フェードして消える
  - **どんより**（`Gloom`）: 頭の上の雨雲と雨粒＋背中に垂れる縦線
