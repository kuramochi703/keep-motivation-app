...
| `models/stage/build.py` | 背景の部屋（床・壁・窓・本・植木鉢）の原典。形も色も、Cycles で焼き込む光と影も全部この1本が作る | 部屋の形・色・明るさ |
...
| 気分 | 6段階（連続／放置サイクルから計算。しずみこみは停止中） | 彩度・明度・表情・姿勢 | `state/logic.ts` の `MOODS` |
...
**色が変わるのは落ち込んだときだけ。** すこし元気〜かがやきは、いきいきの色（56 / 80）のまま変えない
（色選択画面の見本と同じ色）。うつむきは彩度だけ少し落とし、ぐったりは彩度で沈める。
**明度は 58 より下げない。** 元気さの差は、動き・表情・エフェクトで見せる。

| 気分 | 条件 | 彩度 / 明度 | 目 | `liveliness` | 動き | 叩いたとき | エフェクト |
| --- | --- | --- | --- | --- | --- | --- | --- |
| ~~しずみこみ~~ | **停止中**（`SINK_MOOD` に定義だけ残す） | 4 / 58 | 閉じる | 0 | 伏せて動かない（`Sink`） | 反応する | どんより |
| ぐったり | 3サイクル以上放置 | 8 / 58 | 閉じる | 0 | 歩かず、うずくまる（`Slump`） | **反応しない**（`pokeable: false`） | どんより |
| うつむき | 2サイクル放置 | 40 / 80 | 半目 | 0.2 | 歩かず、休憩（`Rest`）だけ（`restOnly: true`） | 反応する | — |
| すこし元気 | 連続1サイクル | 56 / 80 | まるい目 | 0.4 | 歩く・休む。**跳ばない**（`jumps: false`） | 反応する | — |
| 元気 | 連続2サイクル | 56 / 80 | まるい目 | 0.6 | 歩く・跳ぶ・休む。背筋まっすぐ | 反応する | — |
| いきいき | 連続3〜6サイクル | 56 / 80 | にっこり ∩ | 0.9 | 歩く・跳ぶ・休む | 反応する | 光の粒 |
| かがやき | 連続7サイクル以上 | 56 / 80 | にっこり ∩ | 1 | 歩く・跳ぶ・休む | 反応する | 輝き＋光の粒 |

- 前かがみ（`droop`）は `liveliness` から出す。ただし**すこし元気〜かがやき（`upright: true`）はまっすぐ**。前かがみは落ち込んだときだけの姿勢
- `liveliness` が動きに効くのは「0.15 を超えるか」（歩くかどうか）だけ（`Chick.tsx` の `pick()`）。
  超えていれば歩く・休む頻度は同じ。跳ぶかどうかは `jumps` で決める（**すこし元気は跳ばない**。元気との差はここ）
- うつむき（0.2）は閾値を超えているが、`restOnly` で歩かず座り込みだけにしている
- 座り込む姿勢は気分で分ける（`look.ts` の `SIT_OF`）。休憩（`Rest`）は元気なときの
  ひと休みでもあるので、ぐったりには使い回さず、うずくまる（`Slump`）にする
...
10. **キャンバスの中で glb を読むときは、先読み（`useGLTF.preload`）とキャンバス内の `Suspense` をセットで。**
   読み込み待ちが外（`Avatar.tsx`）の `Suspense` まで届くと、もう出ているひよこごとキャンバスが
   `display:none` で隠れて枠が一色になり、環境によっては WebGL のコンテキストが落ちて戻らない（部屋の `Stage` で起きた）
11. **ひよこの落ち影（`ContactShadows`）に渡す `scale` は `useMemo` で固定する。** 配列が変わるたびに影の描画先を
   作り直して古いものを捨てないので、毎秒再描画されるダッシュボードでは GPU のメモリが増え続ける。
   また**床はワールドの高さ 0 に置き、カメラの方を動かす**（`StageFit`）。影のぼかしはワールドの原点で
   行われ、床から高さ 2 までしか撮らないので、床を 2 以上下げると影が消える
