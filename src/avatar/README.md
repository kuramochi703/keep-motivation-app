...
| 気分 | 条件 | 彩度 / 明度 | 目 | ほか |
| --- | --- | --- | --- | --- |
| しずみこみ | 4サイクル放置 | 4 / 58 | 閉じる | 伏せて動かない（`Sink`）・あぶら汗 |
| ぐったり | 3サイクル放置 | 8 / 58 | 閉じる | うずくまる（`Slump`）・あぶら汗 |
...
叩くと光の粒（`TapSparkle`）が弾けて、0.9秒で消える。
...
| 弾ける粒（飾り） | コード（`TapSparkle`）。`Sweat` や `Sparkles` と同じ仲間 |
...
3. **「視差効果を減らす」設定のときは弾けさせない。** 飛び散る粒はまさにその設定が避けたい動きなので、
   姿勢のクリップと同じ扱いで止める

粒は**奥行きを見ないで描く**（`depthTest: false`）。からだの中から湧いて出るので、
まじめに前後を見ると半分が胴に隠れて手応えにならない。

叩かれると、粒を弾けさせると同時に `REACTIONS` から**1本を引いて流す**（`react` モード）。
...
9. **叩いて返す動きを足すときは、粒ではなくクリップを増やす。** `TapSparkle` は飾りなので、
   ひよこが喜ぶ・驚くといった姿勢そのものは Blender に1本足して `POSTURE` に並べ、
   `pick()` にワンショット（`Jump` と同じ作り）として入れること
