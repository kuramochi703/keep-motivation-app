...
| 気分 | 6段階（連続／放置サイクルから計算） | 彩度・明度・表情・姿勢 | `state/logic.ts` の `MOODS` |
...
- 座り込む・立ち上がるはモデルに無い。`Idle` と `Rest` の姿勢差が、重みを寄せる間にそのまま動きになる
...
stateDiagram-v2
  ...
  idle --> jump
  idle --> rest: liveliness <= 0.15 なら必ずこれ
...
- 目を書くクリップは `Blink` と `Rest` の2本だけ（`EYE_CLIPS`）。歩きながらまばたきさせるため姿勢の取り合いに入れない
- **`Rest` の重みぶんだけ `Blink` を下げる。** 両方を重み1で流すと、開いた目（1.0）と閉じた目（0.08）が平均されて半目で止まる
